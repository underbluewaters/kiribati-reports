...
/**
 * reefBioregions: A geoprocessing function that calculates overlap metrics for vector datasources
 * @param sketch - A sketch or collection of sketches
 * @param extraParams
 * @returns Calculated metrics and a null sketch
 */
export async function reefBioregions(
  sketch:
  ...
  const metricGroup = project.getMetricGroup("reefBioregions");
  const metrics = (
    await Promise.all(
...
export default new GeoprocessingHandler(reefBioregions, {
  title: "reefBioregions",
  description: "Reef Associated Bioregions",
  timeout: 500, // seconds
